...
export function calcularScoreOutfit(top, bottom, shoes, outerwear = null) {
  ...
  // ── Regla de formalidad flexible ────────────────────────
// Ahora una prenda puede tener varias formalidades:
// ejemplo: "casual,smart"

const formalidadesPrendas = prendas.map((p) =>
  p.formalidad
    .split(',')
    .map((f) => f.trim())
)

// Ver si TODAS las prendas comparten al menos una formalidad
const formalidadesComunes = formalidadesPrendas.reduce((acc, actuales) =>
  acc.filter((f) => actuales.includes(f))
)

if (formalidadesComunes.length === 0) {
  return 0 // outfit inválido
}

// Bonus si todas comparten exactamente el mismo estilo principal
if (formalidadesComunes.length >= 1) {
  score += 10
}    
